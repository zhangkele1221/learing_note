...
DEFINE_string(service_name, "zhangkele", "service name");
...
bool ServerConfiguration::load(const char * configFilePath) {
    ...
    */
    ...
    //FileUtil::readFileContent(configFilePath, configJsonStr);//感觉这个要替换掉.....
    
    try {
    //    FromJsonString(_swConfig, configJsonStr);// 这里要处理 不然 _swConfig 没有初始化......
